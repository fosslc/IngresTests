...
char  *syntax =
 ...
 " ------ -------- ------------------------------ ---------------- -------\n" \
 "   -b    run     Set ticket table count          2 to 64         24\n" \
...
*/
   for (i = 3; i < argc ; i++)
   {
      if (*argv[i] == '-' && *(argv[i]+1) != '\0')
      {
         ...
         {
            case 'B':
               ttables = atoi(argv[i]+2); 
               if (ttables < 2 || ttables > MAXTIXTABLES)
                  ttables = TIXTABLES;
               break;
            case 'D':
...
void dopick(unsigned char *p, unsigned char *pa)
{
   ...
   for (i = 0; i < NUMNUMS; i++)
   {
      EXEC SQL repeated select random(0, :nums - 1) into :pick;
...
*/
      if (reconn == 1)
      {
         ...
         EXEC SQL set lockmode on ltixadmin where
...
**         has expired yet
**      3) A randomly chosen ticket table identifier into which the new
...
/*
** This code is a diagnostic to verify that the random() function used
** above was a legal value (this was used in isolating a bug)
*/ 
      if (tabid < 0 || tabid > maxtt)
      {
         printf("ERROR: Invalid random(0,%d) value: %d\n", maxtt,tabid);
         printf("       tixid: %36s\n", tixid);
         printf("       stopflag: %d\n", stopflag);
...
** variable so statement can't be hard-coded).
*/
      sprintf(sbuff, "insert into ltixtable%02d values (?,?,date('now'),?)",
              tabid); 
...
*/
      EXEC SQL repeated update ltixadmin set soldcount=soldcount+1,
                                            tsoldcount=tsoldcount+1
                        where tblid = :tabid;
      EXEC SQL repeated select tsoldcount, maxsoldcount into :sc, :maxsc
                        from ltixadmin where tblid = :tabid; 
/*
...
*/
      if (sc >= maxsc)
      {
         EXEC SQL repeated update ltixadmin set tsoldcount=0
                        where tblid = :tabid;
         sprintf(sbuff, "raise dbevent ltixtblevent '%02d'", tabid);
...
*/
      EXEC SQL raise dbevent ltixtblevent 'DONE';
      reconn = 0;
   }
   running--;
   if (reconn == 0)
...
void createobjs()
{
   ...
   printf("Cleaning up...\n");
   EXEC SQL drop ltixadmin, ltixtable;
   EXEC SQL drop dbevent ltixtblevent;

   for (i = 0; i < MAXTIXTABLES; i++)
   {
      sprintf(stmtbuff, "drop ltixtable%02d", i);
   ...
   EXEC SQL create dbevent ltixtblevent;

   printf("Creating ticket tables...\n");
   for (i = 0; i < ttables; i++)
   {
      sprintf(stmtbuff,
              "create table ltixtable%02d (tixid char(36), vendor int," \
              "ts date, pix byte(6)) with page_size=4096",
              i);
      EXEC SQL execute immediate :stmtbuff;
   }
   EXEC SQL create table ltixtable (tixid char(36), vendor int,
                         ts date, pix byte(6)) with page_size=4096;

   printf("Creating ticket admin table...\n");
   maxsold = drainage;
   EXEC SQL create table ltixadmin (tblid int, soldcount int,
                   tsoldcount int, maxsoldcount int)
            with page_size=4096;
   for (i = 0; i < ttables; i++) 
   {
      EXEC SQL insert into ltixadmin values (:i, 0, 0, :maxsold); 
   }
   EXEC SQL modify ltixadmin to hash unique on tblid;
...
*/
   ...
   {
      sprintf(sbuff,
              "insert into ltixtable select * from ltixtable%02d", i);
      EXEC SQL execute immediate :sbuff;
      sprintf(sbuff,
              "modify ltixtable%02d to truncated", i);
      EXEC SQL execute immediate :sbuff;
      EXEC SQL commit;
   }
   printf("Sorting/counting tickets...\n");
   EXEC SQL modify ltixtable to hash on pix; 
   EXEC SQL select count(*) into :totalsold from ltixtable;
   printf("Total tickets sold (ticket table): %d\n", totalsold);

   printf("Verifying ticket count...\n");
   EXEC SQL select sum(soldcount) into :admintotal from ltixadmin;
   ...
   EXEC SQL select count(*) into :winners from ltixtable where
            pix = :pix;
   if (winners == 0)
...
*/
   EXEC SQL register dbevent ltixtblevent;
...
*/
      ...
      sprintf(sbuff,
              "insert into ltixtable select * from ltixtable%2.2s", etext);
      EXEC SQL execute immediate :sbuff;
      sprintf(sbuff,
              "modify ltixtable%2.2s to truncated", etext);
      EXEC SQL execute immediate :sbuff;
      EXEC SQL commit;
   }
}

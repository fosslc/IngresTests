...
char  *syntax =
 "\n"
 "Syntax:     inshash <database> <function> [ <option> <option> ... ]\n\n"
 "<database>  target database name\n" \
 "<function>  init    - initialize inshash database objects\n"
 "            run     - execute inshash program, display results\n"
 "            cleanup - delete all inshash database objects\n"
...
*/
   ...
   EXEC SQL select count(*), max(keyval)
            into :initrows, :currhighkey from inshashtbl1;
...
retry1:
      ...
      if (opr == 1)  /* insert */
      {
         EXEC SQL repeated insert into inshashtbl1 values (
                     random(:lk, :hk),
      ...
      {
         EXEC SQL repeated delete from inshashtbl1
                  where keyval = :dval;
...
void cleanup()
{
   ...
   EXEC SQL drop table inshashtbl1;
   EXEC SQL commit;
...
void createobjs()
{
   ...
   EXEC SQL create table inshashtbl1
   ...
   EXEC SQL modify inshashtbl1 to hash on keyval;

   if (secondary)
   {
      EXEC SQL create index inshashtbl1_idx on inshashtbl1 (data1)
               with persistence, structure = hash, page_size=4096;
   }
   if (parts > 1)
   {
      sprintf(stmtbuff,
              "modify inshashtbl1 to reconstruct with partition="
...
void checkit()
{
   ...
   EXEC SQL select count(*), min(keyval), max(keyval) into
            :totrows, :minkey, :maxkey from inshashtbl1;

...
**
**  zsumv2.sc - single account table variant
**
**  History:
**
**  17-May-2004 sarjo01: Created
...
char  *syntax =
 "\n" \
 "Syntax:     zsumv2 <database> <function> [ <option> <option> ... ]\n\n" \
 "<database>  target database name\n" \
 "<function>  init   - initialize database\n" \
 "            run    - execute zsumv2 program, display results\n" \
 "<option>    program option of the form -x[value]\n\n" \
 " Option Function Description                    Param Values     Default\n" \
 " ------ -------- ------------------------------ ---------------- -------\n" \
 "   -a    init    Set no. of account records      1 to 500000     10000\n" \
 ...
 "                                                 S(mall)\n" \
 "   -t    run     Set no. of client threads       1 to 128        8\n" \
 "   -u    init    Specify unique keys             none            disabled\n" \
 "   -v    run     Enable verbose output mode      none            disabled\n" \
 "   -x    run     Set no. of transactions per     1 to 10000,     0\n" \
...
*/
   sprintf(cmdline, "threads: %d, xacts: %d, x/conn: %d, locks: %c",
           nthreads, iters, xperconn, ltype); 
...
*/ 
   printf("Cleaning up...\n");
   exec sql update zsumv2acct1 set rxacts = 0;
   exec sql update zsumv2admin1 set t1 = 'now', cmdline = :cmdline;
   exec sql select accts into :numaccts from zsumv2admin1;
...
** integrity check.
*/
   exec sql update zsumv2admin1 set t2 = 'now';
   checkit();
   exec sql disconnect 'mastercon';
   printf("\n");
...
*/
      EXEC SQL repeated select random(0, :accts - 1) into :acctid; 
/*
...
*/
      EXEC SQL repeated select a1.bal, a2.bal into :bal1, :bal2
               from zsumv2acct1 a1, zsumv2acct1 a2
               where a1.accid = :acctid and a2.accid = :acctid + :accts;
      if (sqlca.sqlcode > 0)
...
*/
      if (bal1 > 0.0)
      {
         EXEC SQL repeated update zsumv2acct1 set bal = bal - 1.00,
         ...
         EXEC SQL repeated update zsumv2acct1 set bal = bal + 1.00,
                                  xacts = xacts + 1, rxacts = rxacts + 1
                         where accid = :acctid + :accts;
         if (sqlca.sqlcode > 0)
            fatalerr(3, acctid, sqlca.sqlcode);
      } 
      else
      {
         EXEC SQL repeated update zsumv2acct1 set bal = money(:bal2),
                                  xacts = xacts + 1, rxacts = rxacts + 1
                           where accid = :acctid;
         if (sqlca.sqlcode > 0)
            fatalerr(4, acctid, sqlca.sqlcode);
         EXEC SQL repeated update zsumv2acct1 set bal = money(:bal1),
                                  xacts = xacts + 1, rxacts = rxacts + 1
                           where accid = :acctid + :accts;
         if (sqlca.sqlcode > 0)
...
void createobjs(int count)
{
   ...
   printf("Cleaning up...\n");
   EXEC SQL drop zsumv2admin1, zsumv2acct1;
   EXEC SQL drop view zsumv2acct1_v;

   EXEC SQL WHENEVER SQLERROR STOP;

   printf("Creating table zsumv2admin1...\n");
   EXEC SQL create table zsumv2admin1 (accts int, xacts int, rsize char(1),
                                 useidx int, tstruct char(1), uniquevals int, 
                                 t1 date, t2 date, cmdline char(256));

   printf("Creating table zsumv2acct1...\n");
   if (*rsize == 'L')
   {
      EXEC SQL create table zsumv2acct1
   ...
   EXEC SQL insert into zsumv2admin1 values
            (:accts, 0, :rsize, :useidx, :tstruct,
             :uniquevals, 'now', 'now', '');

   for (i = 0; i < count*2; i++)
   {
      EXEC SQL REPEATED INSERT into zsumv2acct1 values
   ...
   printf("Updating account records...\n");
   EXEC SQL update zsumv2acct1 set bal = money(0.0) where 
            accid >= :accts;

   EXEC SQL create view zsumv2acct1_v as
            select * from zsumv2acct1 where accid > 
                          (select ((max(accid)+1)/2)-1 from zsumv2acct1); 
...
*/ 
   EXEC SQL select accts, rsize, t1, t2, t2 - t1,
            ...
            into :accts, :c1, :t1, :t2, :t3, :x4, :cmdline,
                 :c2, :x5, :c3, :x6  from zsumv2admin1;

   EXEC SQL select count(distinct a1.bal + a2.bal)
                   into :dbals from zsumv2acct1 a1, zsumv2acct1_v a2
                   where (a1.accid + :accts) = a2.accid;

   EXEC SQL select distinct a1.bal + a2.bal into :bal
                   from zsumv2acct1 a1, zsumv2acct1_v a2
                   where (a1.accid + :accts) = a2.accid and a1.accid = 0; 

   EXEC SQL select sum(rxacts) into :x1 from zsumv2acct1
            where accid < :accts;
   EXEC SQL select sum(rxacts) into :x2 from zsumv2acct1_v;
   ...
   if (bal != BALANCE)
   {
      printf("\nError: Incorrect balance. Expected value: $%5.2f\n", bal);
      return;
   } 

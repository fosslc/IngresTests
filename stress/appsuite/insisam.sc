...
char  *syntax =
 "\n"
 "Syntax:     insisam <database> <function> [ <option> <option> ... ]\n\n"
 "<database>  target database name\n" \
 "<function>  init    - initialize insisam database objects\n"
 "            run     - execute insisam program, display results\n"
 "            cleanup - delete all insisam database objects\n"
...
*/
   ...
   EXEC SQL select count(*), max(keyval)
            into :initrows, :currhighkey from insisamtbl1;
...
retry1:
      ...
      if (opr == 1)  /* insert */
      {
         EXEC SQL repeated insert into insisamtbl1 values (
                     random(:lk, :hk),
      ...
      {
         EXEC SQL repeated delete from insisamtbl1
                  where keyval = :dval;
...
void cleanup()
{
   ...
   EXEC SQL drop table insisamtbl1;
   EXEC SQL commit;
...
void createobjs()
{
   ...
   EXEC SQL create table insisamtbl1
   ...
   EXEC SQL modify insisamtbl1 to isam on keyval;

   if (secondary)
   {
      EXEC SQL create index insisamtbl1_idx on insisamtbl1 (data1)
               with persistence, structure = isam, page_size=4096;
   }
   if (parts > 1)
   {
      sprintf(stmtbuff,
              "modify insisamtbl1 to reconstruct with partition="
...
void checkit()
{
   ...
   EXEC SQL select count(*), min(keyval), max(keyval) into
            :totrows, :minkey, :maxkey from insisamtbl1;

...
char  *syntax =
 "\n"
 "Syntax:     ddlv1 <database> <function> [ <option> <option> ... ]\n\n"
 "<database>  target database name\n" \
 "<function>  init    - initialize ddlv1 database objects\n"
 "            run     - execute ddlv1 program, display results\n"
 "            cleanup - delete all ddlv1 database objects\n"
...
retry2:
      if (reconn == 1)
      {
         ...
         EXEC SQL declare global temporary table session.tmpddlv1
...
*/
      ...
      sprintf(tblname, "ddlv1_t%02d_tbl%04d", pval, idx);
...
retry1:
      ...
      if (atbl[idx - 1] == 1) 
      {
         ...
         sprintf(stmtbuff,
                 ...
                 "* from session.tmpddlv1 with page_size=4096",
                 tblname);
         EXEC SQL execute immediate :stmtbuff;
         q = 2;
         sprintf(stmtbuff,
      ...
      if (verbose == MAXVERBOSE)
      {
         ...
         pval, i+1, (cmd == 2)?"DROP  ":"CREATE", tblname);
      }
...
void cleanup()
{
   ...
   for (i = 1; i <= 1000; i++)
   {
      ...
      {
         sprintf(stmtbuff,
                 "drop table ddlv1_t%02d_tbl%04d",
                 j, i);
         EXEC SQL execute immediate :stmtbuff;
      }
   }

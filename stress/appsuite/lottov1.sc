...
char  *syntax =
 "\n" \
 "Syntax:     lottov1 <database> <function> [ <option> <option> ... ]\n\n" \
 "<database>  target database name\n" \
 "<function>  run    - execute lottov1 program, display results\n" \
 ...
 "   -x    run     Set no. of transactions per     1 to 1000       10\n" \
...
*/
      if (reconn == 1)
      {
         ...
         EXEC SQL set lockmode on lv1tixadmin where
...
** variable, based on the thread's ID).
*/
      sprintf(sbuff, "insert into lv1tixtable%02d values (?,?,date('now'),?)",
              tabid); 
...
*/
      EXEC SQL repeated update lv1tixadmin set soldcount=soldcount+1,
                                            tsoldcount=tsoldcount+1
                        where tblid = :tabid;
      EXEC SQL repeated select tsoldcount, maxsoldcount into :sc, :maxsc
                        from lv1tixadmin where tblid = :tabid; 
/*
...
*/
      if (sc >= maxsc)
      {
         EXEC SQL update lv1tixadmin set tsoldcount=0
                        where tblid = :tabid;
         sprintf(sbuff, "raise dbevent lv1tixtblevent '%02d'", tabid);
...
*/
      EXEC SQL raise dbevent lv1tixtblevent 'DONE';
      reconn = 0;
   }
   running--;
   if (reconn == 0)
...
void createobjs()
{
   ...
   printf("Cleaning up...\n");
   EXEC SQL drop lv1tixadmin, lv1tixtable;
   EXEC SQL drop dbevent lv1tixtblevent;

   for (i = 0; i < sellers; i++)
   {
      sprintf(stmtbuff, "drop lv1tixtable%02d", i);
   ...
   EXEC SQL create dbevent lv1tixtblevent;

   printf("Creating ticket tables...\n");
   for (i = 0; i < sellers; i++)
   {
      sprintf(stmtbuff,
              "create table lv1tixtable%02d (tixid char(36), vendor int," \
              "ts date, pix byte(6)) with page_size=4096",
              i);
      EXEC SQL execute immediate :stmtbuff;
   }
   EXEC SQL create table lv1tixtable (tixid char(36), vendor int,
                         ts date, pix byte(6)) with page_size=4096;

   printf("Creating ticket admin table...\n");
   maxsold = drainage;
   EXEC SQL create table lv1tixadmin (tblid int, soldcount int,
                   tsoldcount int, maxsoldcount int)
            with page_size=4096;
   for (i = 0; i < sellers; i++) 
   {
      EXEC SQL insert into lv1tixadmin values (:i, 0, 0, :maxsold); 
   }
   EXEC SQL modify lv1tixadmin to hash unique on tblid;
...
*/
   ...
   {
      sprintf(sbuff,
              "insert into lv1tixtable select * from lv1tixtable%02d", i);
      EXEC SQL execute immediate :sbuff;
      sprintf(sbuff,
              "modify lv1tixtable%02d to truncated", i);
      EXEC SQL execute immediate :sbuff;
      EXEC SQL commit;
   }
   printf("Sorting/counting tickets...\n");
   EXEC SQL modify lv1tixtable to hash on pix; 
   EXEC SQL select count(*) into :totalsold from lv1tixtable;
   printf("Total tickets sold (ticket table): %d\n", totalsold);

   printf("Verifying ticket count...\n");
   EXEC SQL select sum(soldcount) into :admintotal from lv1tixadmin;
   ...
   EXEC SQL select count(*) into :winners from lv1tixtable where
            pix = :pix;
   if (winners == 0)
...
*/
   EXEC SQL register dbevent lv1tixtblevent;
...
*/
      ...
      sprintf(sbuff,
              "insert into lv1tixtable select * from lv1tixtable%2.2s", etext);
      EXEC SQL execute immediate :sbuff;
      sprintf(sbuff,
              "modify lv1tixtable%2.2s to truncated", etext);
      EXEC SQL execute immediate :sbuff;
      EXEC SQL commit;
   }
}
